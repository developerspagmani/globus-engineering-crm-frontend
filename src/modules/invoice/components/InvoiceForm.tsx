...
const InvoiceForm: React.FC<InvoiceFormProps> = ({ initialData, mode }) => {
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
    ...
    if (name === 'customerId') {
      ...
      setFormData((prev: any) => ({ ...prev, [name]: value, customerName: customer?.name || '', address: customer?.street1 || '' }));
    } else {
        ...
        <div className="mb-5">
           <div className="row mb-4 align-items-center">
              <div className="col-md-2">
                 <span className="text-muted fw-semibold">Customer Details</span>
              </div>
              <div className="col-md-10">
                 <div className="p-2 border-bottom text-dark fw-bold">
                    {formData.customerName || 'Select Customer...'} {formData.address && `, ${formData.address}`}
                 </div>
              </div>
           </div>

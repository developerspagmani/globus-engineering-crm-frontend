...
const PaymentReportPage = () => {
  ...
  useEffect(() => {
    setMounted(true);
    if (activeCompany?.id) { (dispatch as any)(fetchVouchers(activeCompany.id)); (dispatch as any)(fetchPendingPayments(activeCompany.id)); }
  ...
  const paymentData = vouchers.filter(v => v.type === 'receipt' && (v.partyName?.toLowerCase() ?? '').includes(searchTerm.toLowerCase()));
  ...
  return (
    <div className="container-fluid py-4 bg-light min-vh-100 animate-fade-in">
      <div className="d-flex justify-content-between align-items-center mb-4 px-2">
        ...
        <button className="btn btn-white shadow-sm border border-light px-3" onClick={() => { (dispatch as any)(fetchVouchers(activeCompany?.id)); (dispatch as any)(fetchPendingPayments(activeCompany?.id)); }}>REFRESH</button>
      </div>
      ...
      <div className="card shadow-sm border-0 mb-4 rounded-4 overflow-hidden">
        <div className="card-body p-3"><div className="row g-3 align-items-center">
          <div className={`col-md-${activeTab === 'PENDING' ? '4' : '5'}`}><input type="text" className="form-control border-0 bg-light-soft px-3" placeholder="Search customer name..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} style={{ height: '42px', borderRadius: '10px' }} /></div>
          {activeTab === 'PENDING' && <div className="col-md-2"><select className="form-select border-0 bg-light-soft" value={ageingFilter} onChange={(e) => setAgeingFilter(e.target.value as any)} style={{ height: '42px', borderRadius: '10px' }}><option value="all">Every Bucket</option><option value="0-30">0-30 Days</option><option value="31-60">31-60 Days</option><option value="61-90">61-90 Days</option><option value="90+">90+ Days</option></select></div>}
          <div className="col-auto ms-auto d-flex justify-content-end"><ReportActions /></div>
        </div></div>
      </div>
      ...
      <div className="card shadow-sm border-0 rounded-4 overflow-hidden"><div className="card-body p-0"><div className="table-responsive" style={{ minHeight: '400px', paddingBottom: '80px' }}>
        <table className="table table-hover align-middle mb-0">
          <thead className="bg-light"><tr className="text-uppercase small fw-bold text-muted">
          ...
          <tbody>
            {(vLoading || pLoading) ? <tr><td colSpan={7}><Loader text="Loading..." /></td></tr> : (activeTab === 'PAYMENT' ? paymentData : filteredPending).map((item: any, idx) => (
              <tr key={item.id} className="border-bottom border-light">
                ...
                <td className="text-center"><div className="d-flex justify-content-center gap-1"><Link href={activeTab === 'PAYMENT' ? `/vouchers/${item.id}/edit` : `/invoices/${item.id}`} className="btn-action-view"><i className="bi bi-eye-fill"></i></Link><div className="dropdown"><button className="btn btn-sm btn-outline-secondary border-0 p-0" data-bs-toggle="dropdown" style={{ width: '32px', height: '32px' }}><i className="bi bi-three-dots-vertical fs-5"></i></button><ul className="dropdown-menu dropdown-menu-end shadow-sm border-0 py-2"><li><button className="dropdown-item d-flex align-items-center gap-2 py-2 small" onClick={() => handlePrintRecord(item, activeTab)}><i className="bi bi-printer text-primary"></i> Quick Print</button></li><li><button className="dropdown-item d-flex align-items-center gap-2 py-2 small" onClick={() => handleExportPDFRecord(item, activeTab)}><i className="bi bi-file-earmark-pdf text-danger"></i> Export PDF</button></li></ul></div></div></td>
              </tr>
            ))}
          </tbody>
        </table>

...
const InwardReportPage = () => {
  ...
  const filteredItems = items.filter(item => {
    ...
    return (item.customerName || item.vendorName || '').toLowerCase().includes(term) || (item.dcNo || item.challanNo || '').toLowerCase().includes(term);
  });
      ...
      <div className="card border-0 shadow-sm mb-4 overflow-hidden rounded-4">
        <div className="card-body p-3">
          <div className="row g-3 align-items-center">
            ...
            <div className="col-lg-2 col-md-3"><select className="form-select border-0 bg-light-soft" style={{ height: '42px', borderRadius: '10px' }}><option>All Inwards</option></select></div>
        ...
        <div className="card-body p-0">
          <div className="table-responsive" style={{ minHeight: '400px', paddingBottom: '80px' }}>
            <table className="table table-hover align-middle mb-0">
              ...
              <tbody>
                {loading ? (<tr><td colSpan={7}><Loader text="Compiling Data..." /></td></tr>) : filteredItems.map((item, index) => (

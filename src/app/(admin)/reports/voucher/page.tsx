...
const VoucherReportPage = () => {
  ...
  const filteredItems = items.filter(v => (v.partyName?.toLowerCase() ?? '').includes(searchTerm.toLowerCase()) || (v.voucherNo?.toLowerCase() ?? '').includes(searchTerm.toLowerCase()) || (v.chequeNo?.toLowerCase() ?? '').includes(searchTerm.toLowerCase()));
  ...
  return (
    <div className="container-fluid py-4 bg-light min-vh-100 animate-fade-in">
      ...
      <div className="card shadow-sm border-0 mb-4 rounded-4 overflow-hidden"><div className="card-body p-3"><div className="row g-3 align-items-center"><div className="col-md-5"><input type="text" className="form-control border-0 bg-light-soft px-3" placeholder="Search party or voucher..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} style={{ height: '42px', borderRadius: '10px' }} /></div><div className="col-md-2"><select className="form-select border-0 bg-light-soft" style={{ height: '42px', borderRadius: '10px' }}><option>All Vouchers</option></select></div><div className="col-auto ms-auto d-flex justify-content-end"><ReportActions /></div></div></div></div>
      ...
      <div className="card shadow-sm border-0 rounded-4 overflow-hidden"><div className="card-body p-0"><div className="table-responsive" style={{ minHeight: '400px', paddingBottom: '80px' }}>
        <table className="table table-hover align-middle mb-0"><thead className="bg-light"><tr className="text-uppercase small fw-bold text-muted">
          ...
          <tbody>
            {loading ? <tr><td colSpan={7}><Loader text="Loading..." /></td></tr> : filteredItems.map((v, i) => (
              <tr key={v.id} className="border-bottom border-light">
                ...
                <td className="text-center"><div className="d-flex justify-content-center gap-1"><Link href={`/vouchers/${v.id}/edit`} className="btn-action-view"><i className="bi bi-eye-fill"></i></Link><div className="dropdown"><button className="btn btn-sm btn-outline-secondary border-0 p-0" data-bs-toggle="dropdown" style={{ width: '32px', height: '32px' }}><i className="bi bi-three-dots-vertical fs-5"></i></button><ul className="dropdown-menu dropdown-menu-end shadow-sm border-0 py-2"><li><button className="dropdown-item d-flex align-items-center gap-2 py-2 small" onClick={() => handlePrint(v)}><i className="bi bi-printer text-primary"></i> Quick Print</button></li><li><button className="dropdown-item d-flex align-items-center gap-2 py-2 small" onClick={() => handleExport(v)}><i className="bi bi-file-earmark-pdf text-danger"></i> Export PDF</button></li></ul></div></div></td>
              </tr>
            ))}
          </tbody>
        </table>
